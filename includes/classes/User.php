...
class User {
	...
	public function getFollowingList() {
		$following_array_string = $this->user['friend_array']; //Get friend array string from table
		$following_array_string = trim($following_array_string, ","); //Remove first and last comma
		return explode(",", $following_array_string); //Split to array at each comma
	}

	public function getFollowerList() {
		$username = $this->user['username'];
		$query = mysqli_query($this->con, "SELECT username FROM users WHERE friend_array Like '%$username%'");
